...
(function($) {
    ...
    $.jqplot.CanvasOverlay = function(opts){
        ...
        this.behindGrid = opts.behindGrid || false;
        this.afterGrid = opts.afterGrid || false;
        if (options.objects) {
                    ...
                    switch (obj.type) {
                        ...
                        case 'rectangle':
                           ...
                           if(xstop != null && xstart != null && ystop != null && ystart != null)
                              ...
                              if(obj.options.rectangleTitle !== "")
                                 {
                                 this.canvas._ctx.save();
                                 this.canvas._ctx.globalOpacity = 1;
                                 this.canvas._ctx.textAlign = "center";
                                 this.canvas._ctx.fillStyle = "#444";
                                 this.canvas._ctx.font = "11px sans-serif";

                                 var strings = obj.options.rectangleTitle.split("\n");

                                 for(var i = 0; i < strings.length; i++)
                                    {
                                    this.canvas._ctx.fillText(strings[i], xstart + 0.5 * (xstop - xstart), ystop + i * 13 + 12);
                                    }

                                 this.canvas._ctx.restore();
                                 }
                              }
                            break;
    ...
    $.jqplot.CanvasOverlay.postPlotDraw = function() {
        ...
        if(co.behindGrid === true)
           $(this.target).find(".jqplot-overlayCanvas-canvas").insertBefore($(this.target).find(".jqplot-grid-canvas"));
        else if(co.afterGrid === true)
           $(this.target).find(".jqplot-overlayCanvas-canvas").insertAfter($(this.target).find(".jqplot-grid-canvas"));
    ...
    function showTooltip(plot, obj, gridpos, datapos) {
        ...
        switch (opts.tooltipLocation)
            {
            case "above":
                x = Math.round(gridpos[0] + 0.5 * (obj.gridStop[0] - obj.gridStart[0]) + plot._gridPadding.left - 0.5 * $(elem).width() - opts.tooltipOffset);
                y = plot._gridPadding.top - opts.tooltipOffset - elem.outerHeight(true) - 5;
                break;
            case 'nw':

...
// written by Brian Crenna  (brian.crenna@canada.ca)
// Last updated 2021 06 06 Su
...
vw.time_controls =
   ...
   initialize: function(owner, options)
      ...
      if(options.maxTime)
         this.maxIndex = Math.round(options.maxTime / this.step);
      ...
      this.createControls();
      this.$parent.children(".time").on("mousedown", function() { if(!$(this).hasClass("inactive")) { time_controls[$(this).prop("id").substr(3)](this); } });

      if(this.hasCompareButton === true)
         {
         $("#tc_compare").off("mousedown").on("mousedown", function(e) { time_controls.compare(e.ctrlKey); });
         $("#tc_compare").on("mouseup", function(e) { if(!e.ctrlKey) time_controls.restore(false); });
         }

      $("#time_slider").slider({min: this.step * this.minIndex, max: this.step * this.maxIndex, step: this.step, slide: function(e, ui) { time_controls.update(ui.value); }});

      // initialize the mousewheel to move the time slider when it's over the time container

      $("body").on("mousewheel", function(e) { if(e.deltaY < 0) time_controls.next(); else if(e.deltaY > 0) time_controls.previous(); });
      },
   ...
   reverse: function()
      ...
      if(this.index > 0)
         ...
         this.timeoutID = window.setTimeout(function() { time_controls.reverse(); }, 600);
         }
      else
         {
         this.timeoutID = window.setTimeout(function()
            {
            time_controls.setIndex(time_controls.maxIndex);
            timeoutID = window.setTimeout(function() { time_controls.reverse(); }, 600);
            }, 1500);
         }
   ...
   play: function()
      ...
      if(this.index < this.maxIndex)
         {
         this.setIndex(this.index + 1);
         this.timeoutID = window.setTimeout(function() { time_controls.play(); }, 600);
         }
      else
         {
         this.timeoutID = window.setTimeout(function()
            {
            time_controls.setIndex(0);
            time_controls.timeoutID = window.setTimeout(function() { time_controls.play(); }, 600);
            }, 1500);
         }
   ...
   setStep: function(value)
      ...
      if(Math.abs(this.step - value) > 1.0e-3)
         ...
         this.maxIndex = Math.round(this.maxTime / this.step);
   ...
   update: function(sliderValue)
      ...
      this.hour = this.zeropad(Math.floor(this.step * this.index + 0.0001), this.hourDigits);
   ...
   updateTimeLabel: function()
      ...
      if(this.timestamp !== "")
         ...
         this.timeLabel = date.toISOString().slice(0, -5).replace("T", " ") + "Z";
